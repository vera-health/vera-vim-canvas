...
import type { EhrSectionKey, WriteStatus } from "@/core/types/ehr";

/** Human-readable labels for each SOAP / encounter section */
const SECTION_OPTIONS: { key: EhrSectionKey; label: string }[] = [
...
export function SendToEhrPopover({ containerRef }: { containerRef: React.RefObject<HTMLElement | null> }) {
  ...
  const [lastSection, setLastSection] = useState<EhrSectionKey | null>(null);
  ...
  const handleSendToSection = async (section: EhrSectionKey) => {
    if (!ehrWriter || !selectedText || writeStatus !== "idle") return;
    setLastSection(section);
    try {
      const payload = ehrWriter.buildSectionPayload(section, selectedText);
      await ehrWriter.writeToEncounter(payload);
      // Clear selection
  ...
  if (!ehrWriter || !position || !selectedText) return null;

  // Check if any SOAP section is available
  const anySectionAvailable = SECTION_OPTIONS.some(
    (s) => ehrWriter.getWriteAvailability(s.key) !== "unavailable",
  );

  if (!anySectionAvailable) return null;

  const statusIcon = (section: EhrSectionKey) => {
      ...
      {!showSections ? (
        ...
        >
          ...
          {SECTION_OPTIONS.map(({ key, label }) => {
            const avail = ehrWriter.getWriteAvailability(key);
