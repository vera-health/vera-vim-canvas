...
import { useVeraChat } from "@/core/hooks/useVeraChat";
import { useWhisper } from "@/core/hooks/useWhisper";
import { useNotificationPreferences } from "@/core/hooks/useNotificationPreferences";
import { useExpandState } from "@/core/hooks/useExpandState";
import { Message } from "@/core/components/Message";
import { ReferenceTooltipDisplay } from "@/core/components/renderers/ReferenceTooltip";
import { getSupabase } from "@/core/utils/supabase";
import { ALL_NOTIFICATION_TYPES, NOTIFICATION_TYPE_LABELS } from "@/core/types/notification";
import { Tooltip } from "@/core/components/Tooltip";
import { HelpScreen } from "@/core/components/HelpScreen";
import { SendToEhrPopover } from "@/core/components/SendToEhrPopover";

export interface ChatViewProps {
  ehrContextString: string;
  activeSuggestedQuestions: string[];
...
export function ChatView({
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) {
        ...
        if (input.trim() && !isStreaming) {
          ...
          setInput("");
          sendMessage(text, ehrContextString);
        }
      }
    };
    document.addEventListener("keydown", handler);
    return () => document.removeEventListener("keydown", handler);
  }, [whisperState, isStreaming, micSupported, input, ehrContextString, startRecording, cancelRecording, stopAndTranscribe, sendMessage, toggleExpand]);
  ...
  function handleSampleClick(question: string) {
    ...
    sendMessage(question, ehrContextString);
  }
